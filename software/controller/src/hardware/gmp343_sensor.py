...
CO2_MEASUREMENT_REGEX = (
    ...
    + r"\(R C C\+F T\)")
TIMES_REGEX = r"\d+\.\d+"
STARTUP_REGEX = (f"GMP343 - Version STD {TIMES_REGEX}\\r\\n" +
...
class CO2SensorInterface:
    ...
    def set_compensation_values(self, pressure: float,
                                humidity: float) -> None:
        """
    ...
    def get_current_concentration(
        ...
        try:
            ...
            if (humidity is not None) and (pressure is not None):
                self.set_compensation_values(pressure=pressure,
                                             humidity=humidity)

            answer = self._request_measurement_data()

            xs = [
                s for s in answer.replace("\t", " ").split(" ") if len(s) > 0
            ]
            sensor_data = float(xs[0]), float(xs[1]), float(xs[2]), float(
                xs[3])
            ...
            self.logger.exception(
                e,
                label=
                "Exception during CO2 sensor measurement request - restarting sensor",
                forward=True,
            )
            self._reset_sensor()
    ...
    def _send_command_to_sensor(
        ...
        answer = self.serial_interface.send_command(command, expected_regex,
                                                    timeout)
    ...
    def _request_measurement_data(self) -> str:
        ...
        if answer[0] == "success":
            ...
            raise self.CommunicationError(
                f"Uncomplete. Sensor answer: {answer[1]}")
